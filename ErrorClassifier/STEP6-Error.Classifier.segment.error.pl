            ...
            while( my $line_string = <INPUTMARKDATA> )
            {
                ...
                if( $line_string =~ /$option{ 'token-src' }:(.*)/ )
                {
                    ...
                    foreach my $word ( @words )
                    {
                        ++$current_pos;
                        ++$new_pos;
                        if( $word =~ /\$number/ or $word =~ /\$date/ or $word =~ /\$time/ or $word =~ /'s/ )
                        {
                            ++$new_pos;
#                            print STDERR "$word\n";
...
=cut
                ...
                elsif( $line_string =~ /$option{ 'del-label' }:(.*)/ )
                {
                    ...
                    {
						my $deleted_word_position_tmp;
						if( $tmp_label =~ /(\d+)-(\d+)/ )
						{
							$deleted_word_position_tmp = $1;
						}
						else
						{
							print STDERR "Warning: Format error in $sentence_no sentence in $param{ '-markdata' }.\n";
							next;
						}
                        my $new_deleted_label;
						if( exists $words_label_and_new{ $deleted_word_position_tmp } )
						{
							$new_deleted_label = $words_label_and_new{ $deleted_word_position_tmp };
							++$deleted_labels_new{ $new_deleted_label };
						}
						else
						{
							print STDERR "Warning: Format error in $sentence_no sentence in $param{ '-markdata' }. deleted_pos=$deleted_word_position_tmp\n";
							next;
						}
